import MouseFollower from "../../../islands/MouseFollower.tsx";

export function Hero() {
  return (
    <section
      className={"w-full h-screen text-gray-200"}
    >
      <MouseFollower />

      <div
        className={"w-full h-full backdrop-blur-3xl grid grid-cols-8 items-center justify-center pr-8"}
      >
        <article
          className={"col-span-full p-4 md:p-0 md:col-start-3 md:col-end-6 animate-fade-in-up"}
        >
          <h3 className={"font-black text-primary-10"}>Hi there! My name is</h3>
          <h1
            className={"text-3xl font-black md:text-6xl text-slate-100"}
          >
            Ondřej Tuček.
          </h1>
          <h2 className={"text-3xl md:text-6xl font-black text-slate-300"}>
            I make web apps.
          </h2>
          <p className={"max-w-md mt-4 font-semibold"}>
            I am a computer enthusiast and as such I love linux and producing
            reliable, fast software with bleeding edge technologies.
          </p>
        </article>
      </div>
    </section>
  );
}
